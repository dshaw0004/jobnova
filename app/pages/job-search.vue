...
async function loadJobs() {
  ...
  errorMsg.value = "";
  try {
    const res = await $fetch<{ success: boolean; jobs: any[] }>(
      "/api/jobs/public-list",
      {
        query: {
          ...
          isMsme:
            filters.isMsme !== undefined ? (filters.isMsme ? 1 : 0) : undefined,
        },
      }
    );
    jobs.value = res.jobs;
...
async function submitApplication() {
  ...
  try {
    const res = await $fetch<{
      success: boolean;
      message: string;
      aiScreeningRequired?: boolean;
    }>("/api/jobs/apply", {
      method: "POST",
      body: {
        jobId: applyModal.jobId,
        coverLetter: applyModal.coverLetter || undefined,
      },
    });
    if (res.success) {
      ...
      if (res.aiScreeningRequired) {
        alert(
          "This application requires a brief AI screening interview. Redirecting you to the interview room..."
        );
...
<template>
  <div class="min-h-screen bg-surface flex flex-col">
    <main
      class="mt-16 max-w-7xl mx-auto px-lg py-xl flex flex-col gap-xl w-full"
    >
      <!-- Search Section -->
      <section class="w-full">
        <form
          class="bg-surface-container-lowest p-md rounded-2xl shadow-[0px_4px_20px_rgba(26,115,232,0.08)] flex flex-wrap lg:flex-nowrap items-center gap-base border border-outline-variant/30"
          @submit.prevent="loadJobs"
        >
          <div
            class="flex-grow min-w-[200px] flex items-center gap-sm px-md border-b lg:border-b-0 lg:border-r border-outline-variant/30"
          >
          ...
          <div
            class="flex-grow min-w-[150px] flex items-center gap-sm px-md border-b lg:border-b-0 lg:border-r border-outline-variant/30"
          >
          ...
          <div
            class="flex-grow min-w-[120px] flex items-center gap-sm px-md border-b lg:border-b-0 lg:border-r border-outline-variant/30"
          >
      ...
      <div class="grid grid-cols-12 gap-lg items-start">
        ...
        <aside
          class="hidden lg:flex col-span-3 flex-col gap-md sticky top-24 max-h-[calc(100vh-120px)] overflow-y-auto pr-sm"
        >
          ...
          <!-- Filter Category -->
          <div
            class="bg-surface-container-low p-md rounded-xl flex flex-col gap-sm"
          >
            ...
            <div class="flex flex-col gap-xs mt-xs space-y-2">
              ...
              >
                ...
                />
                <span
                  class="text-body-md text-on-surface-variant group-hover:text-primary transition-colors"
                  >{{ ind }}</span
                >
              </label>
            </div>
          ...
          >
            ...
            <div class="flex flex-col gap-xs mt-xs space-y-2">
              ...
              <label class="flex items-center gap-sm cursor-pointer group">
                ...
                />
                <span
                  class="text-body-md text-on-surface-variant group-hover:text-primary transition-colors flex items-center gap-1"
                  >MSME
                  <span
                    class="bg-amber-100 text-amber-700 px-1 py-0.5 rounded text-[9px] uppercase font-bold tracking-wider"
                    >Highlight</span
                  ></span
                >
              </label>
        ...
        <section class="col-span-12 lg:col-span-5 flex flex-col gap-lg">
          <div>
            ...
            <p class="text-body-lg text-on-surface-variant mt-xs">
              Search and filter through active job openings.
            </p>
          </div>

          <!-- Quick Filters -->
          <div
            class="flex items-center gap-sm overflow-x-auto pb-sm no-scrollbar"
          >
            <button
              class="whitespace-nowrap px-md py-2 bg-surface-container-high text-on-surface-variant rounded-full font-label-md text-label-md hover:bg-secondary-container hover:text-on-secondary-container transition-all"
              @click="setQuickFilter('remote')"
            >
              Remote Jobs
            </button>
            <button
              class="whitespace-nowrap px-md py-2 bg-surface-container-high text-on-surface-variant rounded-full font-label-md text-label-md hover:bg-secondary-container hover:text-on-secondary-container transition-all"
              @click="setQuickFilter('private')"
            >
              IT / Tech Jobs
            </button>
            <button
              class="whitespace-nowrap px-md py-2 bg-amber-500/10 text-amber-700 border border-amber-500/20 rounded-full font-label-md text-label-md hover:bg-amber-500 hover:text-white transition-all flex items-center gap-1.5 shadow-sm"
              @click="setQuickFilter('msme')"
            >
          ...
          <!-- Results Header -->
          <div
            class="flex items-center justify-between border-b border-outline-variant/30 pb-sm"
          >
            <span class="font-label-md text-label-md text-on-surface-variant">
              Showing
              <span class="font-bold text-on-surface">{{ jobs.length }}</span>
              results
            </span>
          </div>
          ...
          <div v-if="loading" class="py-12 flex justify-center">
            <UIcon
              ...
              class="animate-spin text-primary text-[36px]"
            />
          </div>
          ...
          <div v-else class="flex flex-col gap-md">
            <div
              v-if="jobs.length === 0"
              class="py-16 text-center space-y-4 bg-surface-container-low/30 rounded-2xl border border-dashed border-outline-variant"
            >
              <UIcon
                name="i-lucide-briefcase"
                class="text-[48px] text-outline/40"
              />
              <p class="font-body-lg text-on-surface-variant">
                No active jobs match your filters.
              </p>
              <button
                class="text-primary hover:underline font-label-md"
                @click="clearFilters"
              >
                Clear filters
              </button>
            ...
            <article
              ...
              :class="[
                ...
                selectedJob?.id === job.id
                  ...
                  : 'border-outline-variant/30 bg-surface-container-lowest hover:border-primary/50 hover:shadow-md',
              ]"
              @click="selectJob(job)"
            >
              <div
                v-if="job.is_msme === 1"
                class="absolute -top-3 -right-6 bg-gradient-to-r from-amber-500 to-orange-400 text-white text-[9px] font-bold px-8 py-1 rotate-45 shadow-sm tracking-wider z-0"
              >
                MSME
              </div>
                  ...
                  <div>
                    ...
                    <div class="flex items-center gap-sm mt-xs flex-wrap">
                      <NuxtLink
                        ...
                        :class="
                          job.is_msme === 1 ? 'text-amber-600' : 'text-primary'
                        "
                        @click.stop
                      >
                        {{ job.company_name }}
              ...
              <div class="grid grid-cols-2 gap-sm mb-lg">
                ...
                <div class="flex items-center gap-xs text-on-surface-variant">
                  ...
                  <span class="text-body-sm">{{
                    job.ug_qualification || "Any Graduate"
                  }}</span>
                </div>
                <div class="flex items-center gap-xs text-on-surface-variant">
              ...
              <div
                class="flex items-center gap-sm pt-md border-t border-outline-variant/20"
              >
                <button
        ...
        <aside
          class="hidden lg:flex col-span-4 sticky top-24 max-h-[calc(100vh-120px)] overflow-y-auto bg-surface-container-lowest border border-outline-variant/30 rounded-2xl flex-col p-lg shadow-sm"
        >
          <div v-if="selectedJob" class="flex flex-col gap-lg">
            <div class="flex items-center justify-between">
              ...
              >
                <UIcon
                  ...
                  class="text-primary text-[36px]"
                />
              </div>
            </div>
            <div>
              <h2
                class="font-display-lg text-[24px] text-on-surface leading-tight"
              >
                {{ selectedJob.title }}
              </h2>
            ...
            <div class="space-y-md border-t border-outline-variant/20 pt-lg">
              <h4
                class="font-label-md text-label-md text-on-surface font-bold uppercase tracking-wider"
              >
                Job Description
              </h4>
              <p
            ...
            <div class="space-y-md">
              ...
              <div class="grid grid-cols-2 gap-md">
                <div class="p-sm bg-surface-container-low rounded-lg">
                  <span class="block text-body-xs text-outline mb-xs"
                    >Compensation</span
                  >
                  <span class="font-bold text-on-surface">{{
                    formatSalary(selectedJob.sal_min, selectedJob.sal_max)
                  }}</span>
                </div>
                <div class="p-sm bg-surface-container-low rounded-lg">
                  <span class="block text-body-xs text-outline mb-xs"
                  ...
                  >
                  <span class="font-bold text-on-surface">{{
                    formatExperience(selectedJob.exp_min, selectedJob.exp_max)
                  }}</span>
                </div>
              </div>
            </div>
            ...
            <div
              class="p-md bg-green-500/10 text-green-700 rounded-xl flex items-center gap-md border border-green-500/20"
            >
              <UIcon
                name="i-lucide-check-circle"
                class="text-[32px] text-green-600"
              />
              <div>
                <p class="font-bold text-label-md">Verified Employer</p>
                <p class="text-body-sm opacity-80">
                  This company has been vetted by our safety team.
                </p>
              </div>
            </div>
          </div>
          ...
          <div
            v-else
            class="flex-grow flex flex-col items-center justify-center py-20 text-center text-on-surface-variant/40"
          >
  ...
  <div
    ...
    class="fixed inset-0 z-[200] min-w-md flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
    @click.self="closeApplyModal"
  >
    <div
      class="bg-surface-container-lowest rounded-2xl w-full shadow-2xl border border-outline-variant overflow-hidden"
    >
      <!-- Modal Header -->
      <div
        class="flex items-center justify-between px-6 py-4 border-b border-outline-variant/30"
      >
        <div>
          <h3
          ...
          <p
            class="font-body-sm text-on-surface-variant mt-0.5 text-sm truncate max-w-xs"
          >
            {{ applyModal.jobTitle }}
          </p>
        </div>
      ...
      <!-- Modal Footer -->
      <div
        class="px-6 py-4 border-t border-outline-variant/30 flex justify-end gap-3 bg-surface-container-low/50"
      >
        <button
