...
\begin{itemize}
    ...
    \item \textbf{Unified Jobs Portal}: Supports searching and filtering of both private postings and scraped Indian government listings (e.g. SSC, public sector organizations).
...
JobNova is built using a layered serverless architecture. The client frontend communicates with server-side routes compiled as Cloudflare edge functions. These functions query the Cloudflare D1 SQL database, read/write files from Cloudflare R2, and connect to local or remote AI models via Ollama.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        node distance=2cm,
        layer/.style={rectangle, draw, fill=blue!10, text width=8cm, align=center, rounded corners, minimum height=0.9cm, font=\small, thick},
        arrow/.style={draw, <->, >=stealth, thick}
    ]
        \node [layer, fill=green!10] (client) {Frontend Client (Nuxt 4 / Vue 3 / Tailwind CSS)};
        \node [layer, below of=client, fill=blue!15] (edge) {Edge Serverless Engine (Cloudflare Workers / Pages)};
        \node [layer, below of=edge, xshift=-2.5cm, fill=orange!10, text width=3.8cm] (db) {Cloudflare D1 (SQL)};
        \node [layer, below of=edge, xshift=2.5cm, fill=orange!10, text width=3.8cm] (storage) {Cloudflare R2 (Storage)};
        \node [layer, below of=db, xshift=2.5cm, fill=purple!10] (ai) {AI Inference Layer (Ollama / Granite LLM)};
        
        \path [arrow] (client) -- (edge);
        \path [arrow] (edge) -- (db);
        \path [arrow] (edge) -- (storage);
        \path [arrow] (edge) -- (ai);
    \end{tikzpicture}
    \caption{JobNova Layered Serverless Architecture}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.8\textwidth}{\centering \textit{[Screenshot: Login and Account Registration Selection]}\\ \small Clean selection buttons: "Job Seeker" or "Employer", with login forms.}}
    \caption{Account Authentication and Selection Interface}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.8\textwidth}{\centering \textit{[Screenshot: AI Conversational Profile Builder]}\\ \small Chat panel showing the assistant requesting academic details and the user replying in free text.}}
    \caption{AI Profile Builder Interface}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.8\textwidth}{\centering \textit{[Screenshot: Job Seeker Dashboard and Job Agent]}\\ \small Side-by-side view of active private/govt job listings and the AI Job Agent chat widget.}}
    \caption{Job Seeker Dashboard and AI Agent Widget}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.8\textwidth}{\centering \textit{[Screenshot: Employer Applicant Tracking Dashboard]}\\ \small Table of applicants with their AI screening scores (e.g., 85/100) and collapsible screening summaries.}}
    \caption{Employer Applicant Tracking Interface}
\end{figure}
...
\begin{lstlisting}[language=JavaScript, caption=Backend AI Job Agent API Route (agent.post.ts)]
...
% Chapter 14: SYSTEM SCREENSHOTS
\chapter{SYSTEM SCREENSHOTS}

This chapter presents the actual screenshots of the JobNova platform during various user workflows, showing the client interfaces for both Job Seekers and Employers.
